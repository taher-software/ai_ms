...
I want you to extract the topics discussed and for each topic, extract the relevant clips. Here are the requirements for the extracted clips: 

- The extracted clips must not overlap. 
- Each clip duration must be under 120 seconds. 
- Each clip duration must be more than 20 seconds. 
...
def gpt_call(prompt, model="gpt-4o", max_retries=5):
    """
    Calls the OpenAI GPT model with a given prompt and returns the response.
    Arguments:
        prompt: The prompt to send to the GPT model.
        model: The model to use for the GPT call.
        max_retries: Maximum number of retries for the GPT call in case of failure.
    """
    ...
    raise Exception("Max retries exceeded for GPT call")
...
def topic_chatgpt_semi(prompt, model = "gpt-4o"): 
    messages=[{"role": "system", "content": "You are a helpful assistant."},
        {"role": "user", "content": prompt}]
    response = openai_client.chat.completions.create(model=model,messages=messages)
...
def precompute_topics_auto(vid_id, max_retries = 5): 
    ...
    while retries < max_retries:
        try:
            if token_count > 127000:
                topics = []
                chunks = [transcript[i:i+127000] for i in range(0, len(transcript), 127000)]
                prev_topic_id = 0
    ...
    raise Exception("Max retries exceeded for auto topics")       
...
def precompute_topics(vid_id):
    ...
    try:
        print("using auto topics")    
        topics = precompute_topics_auto(video.id)    
    except Exception as err:
        print("Semi auto topics failed, Using semi auto topics")
        print(err)
        try:
            topics = precompute_topics_semi(video.id)
        except Exception as err:
            print("Auto and semi auto topics failed")
            print(err)
            return
...
def precompute_summary(vid_id):
    ...
    if token_count > 16000:    
        summary = ast.literal_eval(gpt_call(summarization_prompt, model = "gpt-4o"))   
    else:
