...
Generate a new topic title that briefly summarizes the information contained in all topics, in the following format:
topic: <topic label>
"""
...
def combine_topics(prompt, model= "gpt-4o"):
...
def cross_asset_topics(topics_per_video, threshold = 0.6, include_non_visited = False):
    from src.models.video import Video    
    ...
    for i in range(len(topic_titles)-1):
        ...
        if topic_title in visited: 
            ...
            if len(similar_topic_indices) > 0:
                ...
                raw_clips = get_clips(probs = probs, nb_samp = 10, use_caption_offset = True)
