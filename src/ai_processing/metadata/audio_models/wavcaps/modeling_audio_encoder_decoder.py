...
AUDIO_ENCODER_DECODER_START_DOCSTRING = r"""
...
AUDIO_ENCODER_DECODER_INPUTS_DOCSTRING = r"""
    Args:
...
class AudioEncoderDecoderModel(PreTrainedModel):
    r"""
    ...
    """
    config_class = AudioEncoderDecoderConfig
    base_model_prefix = "audio_encoder_decoder"
    main_input_name = "audio_feats"
    ...
    def __init__(
        ...
        if config is None:
            config = AudioEncoderDecoderConfig.from_encoder_decoder_configs(encoder.config, decoder.config)
        else:
        ...
        if encoder is None:
            encoder = AudioEncoderModel(config.encoder)

        if decoder is None and is_pretrained is False:
            decoder = AutoModelForCausalLM.from_config(config.decoder)
        elif decoder is None and is_pretrained:
            decoder = AutoModelForCausalLM.from_pretrained(config.decoder.name_or_path)
        self.encoder = encoder
        self.decoder = decoder

        # self.decoder.cls = nn.Linear(config.decoder.hidden_size,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            loss = loss_fct(shifted_prediction_scores.reshape(-1, self.decoder.config.vocab_size), labels.view(-1))
