...
def add_logo(vid_path, logo_path, output_video_path, position=(0, 0, 0, 0), start=0, end=None):
    ...
    while True:
        ...
        if frame_start <= frame_id < frame_end:    
            y_end = min(y + logo_height, height)
            x_end = min(x + logo_width, width)

            y_start = y if y_end - y >= logo_height else y_end - logo_height
            x_start = x if x_end - x >= logo_width else x_end - logo_width

            roi = slice(y_start, y_end), slice(x_start, x_end)

            if dim == 3:
                logo_alpha = logo[:, :, dim] / 255.0
