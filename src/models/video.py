...
class Video(db.Model):
    ...
    def __repr__(self):
        return f"Video('{self.id}','{self.title}','{self.path}','{self.datetime_created }')"
    ...
    def to_json(self):
        data = {c.name: str(getattr(self, c.name) )
                for c in self.__table__.columns if c.name !='uploaded_time'}
        data['uploaded_time'] = str(getattr(self,'datetime_created' ))
