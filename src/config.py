...
class Config(object):
    ...
    # for Test
    SEGMENT_API_KEY = (
        os.environ.get("SEGMENT_API_KEY") or "mZUmSpkmZKIBZW1uiHMGlh06fDKYLrCG"
    )
    # for prod
    # SEGMENT_API_KEY = '2yoWxtHpU2FwenwoJ8njKoc8atOUKKK9'
    ROOT_FOLDER = os.environ.get(
        "ROOT_FOLDER") or "/home/loukkal/Imaginario-app"
    REDIS_URL = os.environ.get("REDIS_URL") or "redis://"
    RQ_REDIS_URL = os.environ.get("REDIS_URL") or "redis://"
    ROOT_APP_FOLDER = (
        os.environ.get("ROOT_APP_FOLDER") or "/home/loukkal/Imaginario-app"
    )
    WEBSITE_URL = (
        os.environ.get("WEBSITE_URL")
        or os.environ.get("IMGN_WEBSITE_ADDRESS")
        or "https://app.imaginario.ai/"
    )
    SOCKET_URI = os.environ.get("SOCKET_URI") or "ws://localhost:5001"
    PAGINATION_ITEMS = 8

    IMAGINARIO_WEB_URL = os.environ.get("IMGN_WEBSITE_ADDRESS")
    IMAGINARIO_AUTH_JWT_SIGNING_SECRETE = (
        os.environ.get("IMAGINARIO_AUTH_JWT_SIGNING_SECRETE") or "ABDEL"
    )
    IMAGINARIO_AUTH_GOD_MODE_API_KEY = (
        os.environ.get("IMAGINARIO_AUTH_GOD_MODE_API_KEY") or "ABDEL"
    )
    IMAGINARIO_FILESYSTEM_ROOT_DIR = (
        os.environ.get("ROOT_APP_FOLDER") or "/home/loukkal/Imaginario-app"
    )
    ...
    STRIPE_API_KEY = (
        ...
        or "sk_test_51LrioSJ19vUdJG79fTCm7B1DyKqWok8iyQNhz7iILYQn3yLmJh9vNpNKvViBa8svp3mX16KIhOJDWXgrdD7DVi8000Gpe5SnBQ"
    )
    # If you are testing with the CLI, find the secret by running 'stripe listen'
    STRIPE_WEBHOOK = (
        os.environ.get("STRIPE_WEBHOOK")
        or "whsec_00a4769d8de68e644700d1280d58f2c303e2b27406fed6a6f484584982071e5a"
    )

    # Google
    GOOGLE_CLIENT_ID = (
        os.environ.get("GOOGLE_CLIENT_ID")
        or "779544015607-ceksdts3cnn1slrm0c8fs7nskem48chv.apps.googleusercontent.com"
    )

    # Sendgrid
    SENDGRID_API_KEY = os.environ.get("SENDGRID_API_KEY")
    SENDGRID_SENDER_NAME = os.environ.get(
        "SENDGRID_SENDER_NAME") or "Imaginario AI"
    SENDGRID_SENDER_ADDRESS = os.environ.get("SENDGRID_SENDER_ADDRESS") or ""
    SENDGRID_SENDER_CITY = os.environ.get(
        "SENDGRID_SENDER_CITY") or "Philadelphia"
    SENDGRID_SENDER_STATE = (
        os.environ.get(
            "SENDGRID_SENDER_STATE") or "Pennsylvania, United States"
    )
    ...
    MFA_AUTHENTICATOR_ENABLED = os.environ.get(
        "MFA_AUTHENTICATOR_ENABLED") or "0"
    DOWNLOAD_TRAINING_ENABLED = os.environ.get(
        "DOWNLOAD_TRAINING_ENABLED") or "0"
    ...
    # Pipedrive
    pipedrive_api_key = (
        os.environ.get("pipedrive_api_key")
        or "ea031255503a59b0ceb1d8bf77f96aa904ae5148"
    )
    ...
    mediaconvert_region_name = os.environ.get(
        "mediaconvert_region_name") or "us-east-1"
    mediaconvert_arn_role = (
        os.environ.get("mediaconvert_arn_role")
        or "arn:aws:iam::680512422523:role/ElementalMediaConvert"
    )
    mediaconvert_bucket_name = (
        os.environ.get(
            "mediaconvert_bucket_name") or "mediaconvert-transcoding"
    )

    # AI -dialogue processing
    stt_provider = os.environ.get("stt_provider") or "assembly"
    rev_api_key = (
        os.environ.get("rev_api_key")
        or "02q8gPYRenqOdEyuuBlxFVqdJrdWUjX3PApwkcv8nG_oOVFcXVCfooJTSgqemf6olCtvSmwXgOcfJj7iT1LkD9bxXNNy4"
    )
    assembly_api_key = (
        os.environ.get(
            "assembly_api_key") or "187dd5e3cbf5470989764c09417a306a"
    )
    deepgram_api_key = (
        os.environ.get(
            "deepgram_api_key") or "9343538fb4a66bff5a7a0d691230a5ec346ecfe6"
    )
    openai_api_key = os.environ.get("openai_api_key")
    DIALOGUE_EMBEDDING_MODEL = (
        os.environ.get("DIALOGUE_EMBEDDING_MODEL") or "ALL-MINILM-L6-V2"
    )
    ...
    VISUAL_EMBEDDING_MODEL = os.environ.get(
        "VISUAL_EMBEDDING_MODEL") or "CLIP_VIT_B_32"
    FACE_RECOGNITION_EMBEDDING_MODEL = (
        os.environ.get("FACE_RECOGNITION_EMBEDDING_MODEL") or "FACENET_VGG2"
    )

    # AI -audio processing
    AUDIO_EMBEDDING_MODEL = (
        os.environ.get("AUDIO_EMBEDDING_MODEL") or "LARGER_CLAP_GENERAL"
    )

    app_env = os.environ.get("app_env") or "prod"
    email_blacklist = os.environ.get("email_blacklist") or [
        "imaginario", "appstrax"]
    rekognition_access_key = os.environ.get("rekognition_access_key")
    rekognition_secret_key = os.environ.get("rekognition_secret_key")
    rekognition_region_name = os.environ.get(
        "rekognition_region_name") or "us-east-1"
